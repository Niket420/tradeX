...
import { paperPortfolio as initialPortfolio, companies, getCompany } from "@/lib/mock-data";
...
export default function PaperPortfolioPage() {
  const { holdings, addHolding, removeHolding } = usePaperPortfolio();
  const [candidateSymbol, setCandidateSymbol] = useState("");

  const heldSymbols = new Set(holdings.map((h) => h.symbol));
  const candidates = companies.filter((c) => !heldSymbols.has(c.symbol));
    ...
    <div className="flex flex-col gap-4">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          A simulated portfolio for tracking whether signals actually translate into returns. Holdings persist in this browser and feed into
          Backtesting.
        </p>
      </div>
      ...
      <Card className="flex flex-wrap items-center justify-between gap-3 p-4">
        ...
        <div className="flex items-center gap-2">
          <CompanyCombobox companies={candidates} value={candidateSymbol} onChange={setCandidateSymbol} placeholder="Select a company..." />
          <Button size="sm" className="h-8 gap-1.5" disabled={!candidateSymbol} onClick={handleAdd}>
      ...
      <Card className="p-4">
        ...
        {holdings.length === 0 ? (
          <p className="text-xs text-muted-foreground">No holdings yet — add a position above.</p>
        ) : (
                ...
                {holdings.map((h) => {
                  ...
                  return (
                    <tr key={h.symbol} className="border-b border-border/60">
                      ...
                      <td className="py-1.5 pr-4">
                        <Button variant="ghost" size="icon-sm" onClick={() => removeHolding(h.symbol)}>
